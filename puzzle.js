...
function getDailyConfig() {
    ...
    return {
        ...
        rawSeed,
        dateKey,
        fullSeed: buildFullSeed(n, diff, rawSeed),
    };
}
...
function initDailyButton() {
    ...
    btn.addEventListener("click", () => {
        ...
        if (solvedTime)
            setStatus(t("daily-solved-title").replace("{time}", solvedTime));
    });
}
    ...
    requestAnimationFrame(() => {
        ...
        for (let r = 0; r < n; r++) {
            for (let c = 0; c < n; c++) {
                ...
                if (valSpan)
                    valSpan.textContent = s.userBoard[r][c]
                        ? String(s.userBoard[r][c])
                        : "";
        ...
        timerStopped = s.timerStopped ?? false;
        if (!timerStopped) {
        ...
        if (restoredDiff) {
            ...
            if (diffEl) {
                diffEl.value = restoredDiff;
                syncCustomSelect("difficulty", restoredDiff);
            }
...
function showWinBanner(
    ...
    if (el("win-stat-diff"))
        el("win-stat-diff").textContent = diffLabels[diff] ?? diff;
    ...
    startMatrixRain();
    if (_isFlipperTheme) {
        if (denied) flipperSounds.tilt();
        else flipperSounds.win();
        ...
        if (leaderboardRank !== null && !denied) {
            ...
            if (
                ...
                window
                    .supabaseFetchLeaderboard(size, diff, "alltime", 100)
                    .then((rows) => {
...
function hideWinBanner() {
    ...
    synthwaveWin?.stop();
    if (
        document.documentElement.getAttribute("data-theme") === "flipper" &&
...
function renderBoard(puzzle) {
    ...
    for (let r = 0; r < n; r++) {
        for (let c = 0; c < n; c++) {
            ...
            cell.addEventListener("touchend", (e) => { e.preventDefault(); selectCell(r, c); }, { passive: false });
    ...
    if (boardEl) {
        ...
        if (cells.length >= n * (n - 1) + 1)
            cells[n * (n - 1)].classList.add("corner-bl");
...
function resizeBoard() {
    ...
    document.documentElement.style.setProperty(
        "--cell-value-size",
        `${Math.round(cellPx * 0.32 * fontScale)}px`,
    );
    document.documentElement.style.setProperty(
        "--cage-label-size",
        `${Math.round(cellPx * 0.12 * fontScale)}px`,
    );
    document.documentElement.style.setProperty(
        "--note-size",
        `${Math.round(cellPx * 0.1 * fontScale)}px`,
    );
}
...
function setNumber(r, c, v) {
        ...
        for (let i = 0; i < n; i++) {
            if (i !== c && notesBoard[r][i].has(v)) {
                changes.push({
                    r,
                    c: i,
                    prevValue: userBoard[r][i],
                    prevNotes: new Set(notesBoard[r][i]),
                    prevHint: hintBoard[r][i],
                    nextValue: userBoard[r][i],
                    nextNotes: new Set([...notesBoard[r][i]].filter((x) => x !== v)),
                    nextHint: hintBoard[r][i],
                });
            }
            if (i !== r && notesBoard[i][c].has(v)) {
                changes.push({
                    r: i,
                    c,
                    prevValue: userBoard[i][c],
                    prevNotes: new Set(notesBoard[i][c]),
                    prevHint: hintBoard[i][c],
                    nextValue: userBoard[i][c],
                    nextNotes: new Set([...notesBoard[i][c]].filter((x) => x !== v)),
                    nextHint: hintBoard[i][c],
                });
            }
        }
    }
    ...
    if (v !== 0 && !timerStopped) {
        ...
        if (allFilled) {
            const allCorrect = userBoard.every((row, r2) =>
                row.every((val, c2) => val === currentPuzzle.solution[r2][c2]),
            );
...
function validateAll() {
    ...
    if (allCorrect && !timerStopped) {
        ...
        const wasDailyAlreadySolved =
            wasDailyMode &&
            window._dailyDateKey &&
            !!getDailySolvedTime(window._dailyDateKey);
        if (wasDailyMode && window._dailyDateKey) {
            const dTimeStr = formatTime(elapsedSeconds);
            if (!wasDailyAlreadySolved) {
                markDailySolved(window._dailyDateKey, dTimeStr);
                if (typeof checkAchievementsOnDailySolve === 'function') checkAchievementsOnDailySolve();
            }
            window._dailyMode = false;
            const dBtn = document.getElementById("btn-daily");
            if (dBtn) {
                dBtn.dataset.solved = "true";
                dBtn.title = t("daily-solved-title").replace(
                    "{time}",
                    getDailySolvedTime(window._dailyDateKey),
                );
            }
        ...
        const allHints =
            hintBoard &&
            userBoard.every((row, r) =>
                row.every((v, c) => v === 0 || hintBoard[r][c]),
            );
        const isNewBest = recordSolve(n, diff, elapsedSeconds, moveCount);
        const leaderboardRank =
            !competitiveBlocked && !wasDailyAlreadySolved
                ? getLeaderboardRank(elapsedSeconds, n, diff)
                : null;
        const cleanSolve =
            !competitiveBlocked &&
            !solvedByCheat &&
            !debugSolveUsed &&
            !wasDailyAlreadySolved;
        const _theme = document.documentElement.getAttribute("data-theme");
        if (typeof checkAchievementsOnSolve === 'function') {
            const noHintsUsed = !hintBoard || !hintBoard.some(row => row.some(v => v));
            checkAchievementsOnSolve({ size: n, diff, theme: _theme, cleanSolve, noUndo: !undoWasUsed, noHints: noHintsUsed });
        }
        ...
        if (_isSpaceTheme) {
            ...
            const _afterCard =
                leaderboardRank !== null
                    ...
                    : wasDailyMode && !competitiveBlocked
                        ...
                        : !wasDailyMode && !competitiveBlocked && !wasDailyAlreadySolved && !solvedByCheat && !debugSolveUsed
                            ? () => {
                                    checkAndShowGlobalRankPopup(elapsedSeconds, n, diff, currentPuzzle.seed, cleanSolve);
                                }
                            : null;
            showWinBanner(
                timeStr,
            ...
            if (!competitiveBlocked && !wasDailyAlreadySolved && !solvedByCheat && !debugSolveUsed && !_isFlipperThemeSolve) {
                checkAndShowGlobalRankPopup(elapsedSeconds, n, diff, currentPuzzle.seed, cleanSolve);
            }
        }
    }
    ...
    if (active) {
        ...
        if (currentPuzzle) {
            ...
            if (wrongCount > 0) {
                ...
                document
                    .getElementById("clear-invalid-overlay")
                    ?.classList.add("visible");
            }
        }
...
function clearInvalidCells() {
    ...
    if (statusEl)
        setStatus(
            changes.length === 1
                ? t("status-cleared-1")
                : t("status-cleared-n").replace("{n}", changes.length),
        );
}
...
function showCountdown(onComplete) {
    ...
    const themeColors =
        colors[isConsole ? "console" : isDark ? "dark" : "default"];
...
function setTimerVisible(active) {
    ...
    const btn = document.getElementById("btn-timer");
    if (btn) btn.dataset.active = active ? "true" : "false";
    if (active && currentPuzzle && !timerStopped && timerInterval === null)
        startTimer();
    const headerTimer = document.getElementById("timer-display-header");
    if (headerTimer)
        headerTimer.textContent = active ? formatTime(elapsedSeconds) : "";
...
function undo() {
    ...
    for (const s of batch)
        applyCell(s.r, s.c, s.prevValue, s.prevNotes, s.prevHint);
    validateAll();
    updateUndoRedoButtons();
}
...
function redo() {
    ...
    history.push(batch);
    for (const s of batch)
        applyCell(s.r, s.c, s.nextValue, s.nextNotes, s.nextHint);
    validateAll();
    updateUndoRedoButtons();
}
...
document.addEventListener("keydown", (e) => {
    ...
    if (
        document
            .getElementById("leaderboard-entry-overlay")
            ?.classList.contains("visible")
    )
        return;
    if (typeof tutorialModule !== "undefined" && tutorialModule.isActive())
        return;
    ...
    saveStateBatch([
        {
            ...
            prevNotes: new Set(notesBoard[r][c]),
            prevHint: hintBoard[r][c],
            nextValue: currentPuzzle.solution[r][c],
            nextNotes: new Set(),
            nextHint: true,
        },
    ]);
